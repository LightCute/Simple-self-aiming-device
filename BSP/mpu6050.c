...
uint32_t timer;
float yaw_angle = 0;
static double gyro_x_bias = 0;
static double gyro_y_bias = 0;
static double gyro_z_bias = 0;
Kalman_t KalmanX = {
    .Q_angle = 0.001f,
    .Q_bias = 0.003f,
    .R_measure = 0.03f
...
void MPU6050_CalibrateGyro(I2C_HandleTypeDef *I2Cx, uint16_t samples)
{
    ...
    gyro_x_bias = (double)sum_x / samples / 131.0;
    gyro_y_bias = (double)sum_y / samples / 131.0;
    gyro_z_bias = (double)sum_z / samples / 131.0;
}
...
void MPU6050_Read_All(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct)
{
    ...
    DataStruct->Az = DataStruct->Accel_Z_RAW / Accel_Z_corrector;
    DataStruct->Temperature = (float)((int16_t)temp / (float)340.0 + (float)36.53);
    DataStruct->Gx = DataStruct->Gyro_X_RAW / 131.0 - gyro_x_bias;
    DataStruct->Gy = DataStruct->Gyro_Y_RAW / 131.0 - gyro_y_bias;
    DataStruct->Gz = DataStruct->Gyro_Z_RAW / 131.0 - gyro_z_bias;

		if(DataStruct->Gz > -0.3 && DataStruct->Gz < 0.3)
			DataStruct->Gz = 0;
