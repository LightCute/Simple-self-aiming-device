/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32h7xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define POWER_EN_Pin GPIO_PIN_2
#define POWER_EN_GPIO_Port GPIOE
#define BEEP_Pin GPIO_PIN_3
#define BEEP_GPIO_Port GPIOE
#define KEY1_Pin GPIO_PIN_4
#define KEY1_GPIO_Port GPIOE
#define KEY2_Pin GPIO_PIN_5
#define KEY2_GPIO_Port GPIOE
#define KEY3_Pin GPIO_PIN_6
#define KEY3_GPIO_Port GPIOE
#define KEY3_EXTI_IRQn EXTI9_5_IRQn
#define KEY4_Pin GPIO_PIN_13
#define KEY4_GPIO_Port GPIOC
#define KEY4_EXTI_IRQn EXTI15_10_IRQn
#define LE1A_Pin GPIO_PIN_0
#define LE1A_GPIO_Port GPIOA
#define LE2B_Pin GPIO_PIN_1
#define LE2B_GPIO_Port GPIOA
#define PWM_EN_Pin GPIO_PIN_4
#define PWM_EN_GPIO_Port GPIOA
#define RE1A_Pin GPIO_PIN_6
#define RE1A_GPIO_Port GPIOA
#define RE2B_Pin GPIO_PIN_7
#define RE2B_GPIO_Port GPIOA
#define BIN2_Pin GPIO_PIN_2
#define BIN2_GPIO_Port GPIOB
#define RPWMB_Pin GPIO_PIN_9
#define RPWMB_GPIO_Port GPIOE
#define BIN1_Pin GPIO_PIN_10
#define BIN1_GPIO_Port GPIOE
#define LPWMA_Pin GPIO_PIN_11
#define LPWMA_GPIO_Port GPIOE
#define AIN2_Pin GPIO_PIN_12
#define AIN2_GPIO_Port GPIOE
#define AIN1_Pin GPIO_PIN_15
#define AIN1_GPIO_Port GPIOE
#define RED1_Pin GPIO_PIN_13
#define RED1_GPIO_Port GPIOB
#define RED2_Pin GPIO_PIN_14
#define RED2_GPIO_Port GPIOB
#define RED3_Pin GPIO_PIN_15
#define RED3_GPIO_Port GPIOB
#define RED4_Pin GPIO_PIN_8
#define RED4_GPIO_Port GPIOD
#define RED5_Pin GPIO_PIN_9
#define RED5_GPIO_Port GPIOD
#define RED6_Pin GPIO_PIN_10
#define RED6_GPIO_Port GPIOD
#define RED7_Pin GPIO_PIN_11
#define RED7_GPIO_Port GPIOD
#define RED8_Pin GPIO_PIN_8
#define RED8_GPIO_Port GPIOA
#define LED_Pin GPIO_PIN_15
#define LED_GPIO_Port GPIOA
#define SensorL_Pin GPIO_PIN_6
#define SensorL_GPIO_Port GPIOD
#define SensorR_Pin GPIO_PIN_5
#define SensorR_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
